...
#             arduino_data[0:4] = ymin, xmin, ymax, xmax
            ...
            if boxes[i].all() and class_name and not full_data:
                arduino_data[0:4] = int(ymin/3), int(xmin/3), int(ymax/3), int(xmax/3)
                arduino_data[4] = class_id
    ...
    if linesP is not None:
        ...
        if max_len_line != 0 and max_len_line_index is not None:
            ...
            if degree_theta <= 2 and degree_theta >= 0:
                degree_theta = 0
            if not full_data:
                arduino_data[5] = int(degree_theta)
...
def send_to_arduino(data):
    ...
    for byte_data in data:
        ...
        print(f"Đã gửi byte {byte_data}")
    print("Đã gửi data xong")
                
def received_sign():
    try:
        sign = ser.readline().strip()  
        if sign:
            return sign  
        else:
            return None 
    except Exception as e:
        print(f"Error receiving sign: {e}")
        return None
...
unit_thread = threading.Thread(target = unit_camera)
